...
def nag_pd_sync_services(args, logger):
    ...
    if not no_ack_nag:
        logger.warning('No current Nagios service problems found')
    if no_ack_nag:
        logger.warning('Nagios service problems found:\t {0}'
                       .format(len(no_ack_nag)))
    if not host_problems:
        logger.warning('No current Nagios host problems found')
    if host_problems:
        logger.warning('Nagios host problems found: \t {0}'
    ...
    if service_problems:
        logger.warning('Matches between Nagios and PD: {0}'
                       .format(_json_dump(service_problems)))
    ...
    if ack_in_nagios_host:
        logger.warning(ack_in_nagios_host.json())
